...
function uploadFile($fileInput, $uploadDir) {
    ...
    $newName    = uniqid() . "." . $extension;  // Unique name
    $targetPath = $uploadDir . $newName;

    if (move_uploaded_file($_FILES[$fileInput]['tmp_name'], $targetPath)) {
        // Return relative path so it can be accessed from browser
        return $targetPath;
    } else {
        return "";
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Upload files (uploads/ folder should be inside your project folder)
    $uploadDir = "uploads/";  
    ...
    $sql = "INSERT INTO students 
        (`First_name`, `Middle_name`, `Last_name`, `Date_of_Birth`, `Gender`, `Stream`, `Mobile`, `Email`, 
    ...
    mysqli_stmt_bind_param($stmt, "ssssssssssssssssss", 
        $fname, $mname, $lname, $dob, $Gender, $str, $mobile, $email,
        $pob, $blood, $tounge, $religion, $caste_cat, $caste, $state,
        $photo, $aadhaar, $marks);
